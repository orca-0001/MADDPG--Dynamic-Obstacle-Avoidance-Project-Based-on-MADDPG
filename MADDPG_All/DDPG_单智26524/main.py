...
###########==========    2025年2月26日开始建环境,至今2025年5月28日,从冬天改到了夏天,终于从零构建了一个完整的深度强化学习(MADDPG)的多智能体动态避障的环境,剩下的就是调参了
######################    其中艰辛深有体会,收工放假休息一天      ###################
...
if __name__ == "__main__":
    ...
    num_episodes = 3000  # 训练轮数(如果再训练也是训练这么多轮)
...
# 定义需要保存动画的轮次
animation_episodes = [10, 500,1000,1500,2000,2500,2600,2700,2800,2900,2950,2970,2990,2999,3000,20000, 23000,25000, 26000, 27000,27997.28000, 29000,29955,29995,29999,30000, 50000, 70000, 90000, 100000]
...
for episode in tqdm(range(num_episodes), desc="Training Progress", unit="episode"):
    ...
    while not all(dones):  # 只要有一个智能体未终止，环境步进就会继续
        ...
        new_noise = max(0.1, 1.0 - episode / 2500)
        ...
        next_states, rewards, dones, reward_details = env.step(actions, dones)  # 解包奖励细节
        ...
        for i in range(num_agents):
            ...
            if add_to_buffer:
                maddpg.replay_buffer.append((states, actions, rewards, next_states, dones))

        if len(maddpg.replay_buffer) >= 50000 and total_steps % 20 == 0:
            batch = random.sample(maddpg.replay_buffer, 128)
            states_batch, actions_batch, rewards_batch, next_states_batch, dones_batch = zip(*batch)
            for agent_id in range(num_agents):
                experiences = (states_batch, actions_batch, rewards_batch, next_states_batch, dones_batch)
                maddpg.update(experiences, agent_id)
    ...
    if episode_actions is not None:
        ...
        with open(reward_log_path, "w") as f:
            ...
            for step, details in enumerate(episode_reward_details):
                ...
                f.write(f"位置: {np.round(details['agent0']['pos'], 2)},到目标点距离: {details['agent0']['distance_to_target']:.2f},当前总奖励值: {details['agent0_total']:.2f}\n")
